...
const phpLang = (PHP as { php: unknown }).php || PHP;
 ...
 */
function parseClassProperties(node: Parser.SyntaxNode, source: string): FieldSummary[] {
        ...
        if (propChild.type === 'property_element') {
          ...
          if (varNode) {
            ...
            const typeNode = child.children.find(c =>
              c.type === 'type' || c.type === 'union_type' || c.type === 'named_type'
            );
...
function parseClass(node: Parser.SyntaxNode, source: string): StructSummary | null {
  ...
  for (const child of node.children) {
    if (child.type === 'abstract_modifier') modifiers.push('abstract');
    if (child.type === 'final_modifier') modifiers.push('final');
  }
 ...
 */
function parseTrait(node: Parser.SyntaxNode, source: string): TraitSummary | null {
  const name = node.childForFieldName('name')?.text;
  if (!name) return null;
 ...
 */
export function parsePHP(source: string): ParseOutcome {
  try {
            ...
            if (body) {
              for (const child of body.namedChildren) {
                if (child.type === 'enum_case') {
                  ...
                  if (caseName) variants.push(caseName);
                }
              }
            }
            summary.enums.push({
