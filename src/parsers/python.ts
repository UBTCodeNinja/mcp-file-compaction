 ...
 */
function extractDocstring(node: Parser.SyntaxNode, source: string): string | undefined {
 ...
 */
function extractDecorators(node: Parser.SyntaxNode, source: string): string[] {
 ...
 */
function parseFunctionSignature(node: Parser.SyntaxNode, source: string): string {
 ...
 */
function parseFunction(node: Parser.SyntaxNode, source: string): FunctionSummary | null {
  const name = node.childForFieldName('name')?.text;
 ...
 */
function parseClassFields(node: Parser.SyntaxNode, source: string): FieldSummary[] {
 ...
 * Parse class methods
 */
function parseClassMethods(node: Parser.SyntaxNode, source: string): FunctionSummary[] {
  ...
  for (const child of body.namedChildren) {
    ...
    if (child.type === 'decorated_definition') {
      funcNode = child.namedChildren.find(c =>
...
function parseModuleLevelAssignment(
  ...
  source: string
): { constant?: ConstantSummary; typeAlias?: TypeAliasSummary } {
  const result: { constant?: ConstantSummary; typeAlias?: TypeAliasSummary } = {};
  ...
  if (!name || !isPublicName(name)) return result;
 ...
 */
export function parsePython(source: string): ParseOutcome {
  try {
    ...
    for (const node of tree.rootNode.namedChildren) {
      switch (node.type) {
        ...
        case 'decorated_definition': {
          ...
          const innerDef = node.namedChildren.find(c =>
        ...
        case 'expression_statement': {
          ...
          if (expr?.type === 'assignment' || expr?.type === 'annotated_assignment') {
            ...
            if (typeAlias) summary.typeAliases.push(typeAlias);
          }
          break;
        }
