 ...
 */
function extractJSDoc(node: Parser.SyntaxNode, source: string): string | undefined {
  ...
  if (body) {
    for (const child of body.namedChildren) {
      if (child.type === 'method_signature' || child.type === 'call_signature') {
        ...
        if (generics) sig += generics;
        if (params) sig += source.slice(params.startIndex, params.endIndex);
 ...
 * Parse a type alias
 */
function parseTypeAlias(node: Parser.SyntaxNode, source: string): TypeAliasSummary | null {
  const name = node.childForFieldName('name')?.text;
 ...
 */
export function parseTypeScript(source: string, ext: string = '.ts'): ParseOutcome {
  try {
    ...
    function processNode(node: Parser.SyntaxNode, exported: boolean = false) {
      switch (node.type) {
