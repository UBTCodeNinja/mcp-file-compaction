 ...
 */
export async function peekFile(filePath: string): Promise<string> {
  const absolutePath = path.isAbsolute(filePath)
    ? filePath
    : path.resolve(filePath);
...
export async function editFile(
  ...
  if (!content.includes(oldString)) {
    throw new Error(
      `The specified old_string was not found in ${relativePath}. ` +
  ...
  if (occurrences > 1) {
    throw new Error(
      `The specified old_string appears ${occurrences} times in ${relativePath}. ` +
 ...
 */
export async function fileStatus(): Promise<string> {
  const state = getContextState();
  const status = state.getStatus();

  const lines: string[] = ['Context Status', '==============', ''];
  ...
  if (status.summaries.length > 0) {
    ...
    for (const s of status.summaries) {
      ...
      lines.push(
        `  ${s.relativePath.padEnd(40)} ${formatSize(s.summarySize).padStart(8)} ` +
 ...
 */
export async function forgetFile(filePath: string): Promise<string> {
  const absolutePath = path.isAbsolute(filePath)
    ? filePath
    : path.resolve(filePath);
